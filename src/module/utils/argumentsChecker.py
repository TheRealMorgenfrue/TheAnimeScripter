...
def argumentsChecker(args, outputPath, parser):
    if args.list_presets:
        from src.module.utils.presetLogic import listPresets

        listPresets()
        sys.exit()

    if args.preset:
        from src.module.utils.presetLogic import createPreset
    ...
    if args.cleanup:
        from src.module.utils.dependencyHandler import uninstallDependencies
        from src.module.utils.isCudaInit import detectGPUArchitecture, detectNVidiaGPU
    ...
    if args.ae:
        ...
        cs.ADOBE = True
        from src.module.utils.aeComms import startServerInThread
...
def _handleDependencies(args):
    ...
    if not os.path.exists(cs.FFMPEGPATH) or not os.path.exists(cs.FFPROBEPATH):
        from src.module.utils.getFFMPEG import getFFMPEG

        getFFMPEG()
    else:
        from src.module.utils.getFFMPEG import logFfmpegVersion
    ...
    try:
        from src.module.utils.dependencyHandler import DependencyChecker
...
def processURL(args, outputPath):
    ...
    from module.ytdlp import VideoDownloader
