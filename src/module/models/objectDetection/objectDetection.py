...
from src.module.utils.downloadModels import downloadModels, modelsMap, weightsDir
from src.module.utils.logAndPrint import logAndPrint

from module.io.io_buffers import ReadBuffer, WriteBufferFFmpeg
from src.module.utils.cuda_checker import CudaChecker
from src.module.utils.progressBarLogic import ProgressBarLogic
...
class ObjectDetectionDML:
    def __init__(
        ...
        try:
            ...
            self.writeBuffer = WriteBufferFFmpeg(
                self.input,
                self.output,
                self.encodeMethod,
                self.customEncoder,
    ...
    def process(self):
        ...
        with ProgressBarLogic(self.totalFrames) as bar:
            for _ in range(self.totalFrames):
                ...
                frameCount += 1
                bar(1)
                if self.readBuffer.is_read_finished():
                    if self.readBuffer.is_queue_empty():
                        break
...
class ObjectDetectionTensorRT:
    def __init__(
        ...
        from src.module.utils.trtHandler import (
    ...
    def handleModels(self):
        ...
        self.colorsTensor = torch.tensor(
            colors, device=checker.device, dtype=torch.float32
        )
    ...
    def prepareInput(self, frame):
        ...
        inputTensor = torch.nn.functional.interpolate(
            frame,
            size=(self.inputHeight, self.inputWidth),
            mode="bilinear",
            align_corners=False,
        )
        return inputTensor
    ...
    def detect(self, frame):
        ...
        logging.info(
            f"Dummy output max: {self.dummyOutput.max()}, min: {self.dummyOutput.min()}"
        )

        numDetections = (
            outputShape[0] if len(outputShape) > 0 and outputShape[0] > 0 else 0
        )
        ...
        return (
            torch.tensor([], device=checker.device, dtype=torch.long),
            torch.tensor([], device=checker.device, dtype=self.dtype),
            torch.tensor([], device=checker.device, dtype=self.dtype),
        )
    ...
    def drawBoxesTorch(self, frame, boxes, classIds):
        for i in range(boxes.shape[0]):
            ...
            frame[:, :, y1:y2, x1:x2] = (
                color.view(1, 3, 1, 1) * 0.3 + frame[:, :, y1:y2, x1:x2] * 0.7
            )
        return frame
            ...
            if boxes.numel() > 0 and boxes.shape[0] > 0:
                if self.disableAnnotations:
                    ...
                    outputImg = draw_detections(
                        frameNp, boxesNp, confidencesNp, classIdsNp
                    )
                    outputImg = cv2.cvtColor(outputImg, cv2.COLOR_BGR2RGB)
                    frame = (
                        torch.from_numpy(outputImg)
                        .permute(2, 0, 1)
                        .unsqueeze(0)
                        .float()
                        .to(checker.device)
