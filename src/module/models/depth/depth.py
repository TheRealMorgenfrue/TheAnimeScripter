...
class DepthCuda:
    ...
    def handleModels(self):
        ...
        if "distill" in self.depth_method and "large" in self.depth_method:
            from module.models.depth.distillanydepth.modeling.archs.dam.dam import (
                DepthAnything,
            )
...
class VideoDepthAnythingCUDA:
    ...
    def handleModels(self):
        ...
        self.device = "cuda" if checker.cuda_available else "cpu"
...
class VideoDepthAnythingTorch:
    ...
    def handleModels(self):
        ...
        if not os.path.exists(os.path.join(weightsDir, weights_model, self.filename)):
            modelPath = downloadModels(
                model=weights_model,
                half=self.half,
                modelType="pth",
    ...
    def process_nelux(self):
        ...
        with ProgressBarLogic(self.totalFrames) as bar:
            for _ in range(self.totalFrames):
                ...
                if (
                    self.readBuffer.is_read_finished()
                    and self.readBuffer.is_queue_empty()
                ):
                    break
