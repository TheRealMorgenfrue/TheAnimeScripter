...
class OmniShotCut(nn.Module):
    ...
    def __init__(
        ...
        backbone,
        transformer,
        num_intra_relation_classes,
        num_inter_relation_classes,
        num_frames,
        num_queries,
        aux_loss=False,
    ):
        ...
        self.intra_relation_class_embed = nn.Linear(
            hidden_dim, num_intra_relation_classes
        )
        self.inter_relation_class_embed = nn.Linear(
            hidden_dim, num_inter_relation_classes
        )
        self.range_class_embed = nn.Linear(
            hidden_dim, num_frames + 2
        )  # TODO: s+2 is to add some padding
    ...
    def forward(self, samples: NestedTensor):
        ...
        features, pos = self.backbone(
            samples
        )  # Joiner outputs ResNet features and the position embedding
        ...
        src_proj = (
            src_proj.reshape(n // self.num_frames, self.num_frames, c, h, w)
            .permute(0, 2, 1, 3, 4)
            .flatten(-2)
        )
        ...
        hs = self.transformer(
            src_proj, mask, self.query_embed.weight, pos
        )[
            0
        ]  # src_proj shape is (B, C, F, H*W);   mask shape is (B, F, H*W);    pos shape is (B, C, F, H*W)
        ...
        if self.aux_loss:
            out["aux_outputs"] = self._set_aux_loss(
                outputs_intra_class, outputs_inter_class, outputs_shot_class
            )  # Fetch Central Layer (Except the last layer)
    ...
    def _set_aux_loss(
        ...
        return [
            {"intra_clip_logits": a1, "inter_clip_logits": a2, "pred_shot_logits": b}
            for a1, a2, b in zip(
                outputs_intra_class[:-1],
                outputs_inter_class[:-1],
                outputs_shot_class[:-1],
                strict=True,
            )
        ]
