...
class DepthEstimator:
    ...
    def _load_model(self):
        ...
        from .models.depth.dpt_v2 import DepthAnythingV2
...
class DepthGuidedRifeCuda:
    ...
    def _load_rife_model(self):
        ...
        from .vfi.rifearches.IFNet_rife425 import IFNet
    ...
    def _setup_buffers(self):
        ...
        logging.info(
            f"DepthGuidedRifeCuda initialized: {self.width}x{self.height}, "
    ...
    def _compute_depth_visibility(
        ...
        kernel_size = 3
        kernel = torch.ones(
            1, 1, kernel_size, kernel_size, device=self.device, dtype=self.dtype
        )
    ...
    def _interpolate_step(self, timestep: float) -> torch.Tensor:
        ...
        vis_0, vis_1 = self._compute_depth_visibility(
            self.D0, self.D1, flow[:, :2], flow[:, 2:4]
        )

        result = self._depth_guided_blend(warped_img0, warped_img1, mask, vis_0, vis_1)
