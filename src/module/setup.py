...
from src.module.utils.inputOutputHandler import IOHandler
...
def process_arguments():
    ...
    for i, path_config in enumerate(input_files, 1):
        logger.info(f"({i}/{file_count}) Processing video '{path_config}'")
        try:
            VideoProcessor(path_config)
        except Exception:
            logger.error(
                f"Failed to process video '{path_config}'\n{traceback.format_exc()}"
            )

    logger.info(f"Total execution time: {time() - start_time}")
